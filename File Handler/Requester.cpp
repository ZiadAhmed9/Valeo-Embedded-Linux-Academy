...
/*Supported commands For my application*/
string listing_cmd = "ls";    // Listing command
string file_cont_cmd = "cat"; // File contents command
...
{
  int pipe_d = open(PIPE_NAME, O_WRONLY); // Get pipe desciptor in this variable
  ...
  sem_t *semptr = sem_open(SEM_NAME, 0); // this function return pointer to semaphore
...
{
  if (*pipe_d == -1)      //If descriptor acquired equals -1
  {
